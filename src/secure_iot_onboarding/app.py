...
Endpoints:
  POST /register     - Register device, issue cert, provision broker,
                       return credential package link.
  GET /device/<id>   - Get device metadata & certificate info.
  POST /revoke/<id>  - Revoke device certificate & remove broker access.

This module wires together DB (db.py), CA (ca.py), broker provisioning
(broker_provisioning.py), and credential delivery (credential_delivery.py).
"""
...
def create_app() -> Flask:
    ...
    def register():
        try:
            payload = DeviceRegistrationModel(
                **request.get_json(force=True)
            )
        except ValidationError as ve:
            return jsonify(
                {"error": "validation_error", "details": ve.errors()}
        ...
        broker_provisioning.add_device_acl(
            str(device.id), topics=[f"devices/{device.id}/#"]
        )

        # Package credentials
        zip_path = credential_delivery.package_credentials(str(device.id))
        download_link = credential_delivery.generate_download_link(
            zip_path, expiry_minutes=30
        )
            ...
            jsonify(
                {
                    ...
                    "registered_at": (
                        ...
                        if device.registered_at else None
                    ),
                    ...
                    "certificate": {
                        ...
                        "issued_at": (
                            ...
                            if cert and cert.issued_at else None
                        ),
                        "expires_at": (
                            cert.expires_at.isoformat()
                            if cert and cert.expires_at else None
                        ),
    ...
    def revoke(device_id: str):
        ...
        return jsonify(
            {"status": "revoked", "device_id": device_id}
        ), HTTPStatus.OK

    @app.route("/health", methods=["GET"])
    def health():
        return jsonify(
            {"status": "ok", "time": datetime.utcnow().isoformat()}
        ), HTTPStatus.OK
