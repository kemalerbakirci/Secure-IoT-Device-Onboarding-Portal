...
class Device(Base):
	...
	certificate = relationship("Certificate", back_populates="device", uselist=False, cascade="all, delete-orphan")
...
def add_device(name: str, type: str, location: Optional[str] = None, firmware: Optional[str] = None) -> Device:
...
def add_certificate(device_id: str, fingerprint: str, expires_at) -> Certificate:
	session = SessionLocal()
	cert = Certificate(device_id=device_id, fingerprint=fingerprint, expires_at=expires_at)
	session.add(cert)
	session.commit()
