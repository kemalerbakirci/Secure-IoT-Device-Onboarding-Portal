...
def sign_certificate(csr_path: str, device_id) -> Tuple[str, str, datetime]:
	...
	with _lock:
		...
		cert = (
			...
			.add_extension(x509.SubjectAlternativeName([x509.DNSName(str(device_id))]), critical=False)
...
def revoke_certificate(fingerprint: str):
	...
	with _lock:
		...
		if REVOCATION_FILE.exists():
			existing = {line.strip() for line in REVOCATION_FILE.read_text().splitlines() if line.strip()}
...
def create_crl():
	...
	revoked = []
	if REVOCATION_FILE.exists():
		revoked = [line.strip() for line in REVOCATION_FILE.read_text().splitlines() if line.strip()]
	# Simple text-based CRL (not a real X.509 CRL for brevity) – extend as needed
	content = ["-----BEGIN REVOKED FINGERPRINT LIST-----"] + revoked + ["-----END REVOKED FINGERPRINT LIST-----"]
	CRL_FILE.write_text("\n".join(content))
